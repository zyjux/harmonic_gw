...
def read_concat_files(caseDict):
    data = None
    lat = None
    lon = None
    dataMeta = None
    ...
    for filename in fileList:
        ...
        dataMetaT['datetime'] = [str(pd.to_datetime(filename[0:17], format="d%Y%m%d_t%H%M%S"))]
...
def validate_data(data, lat, lon):
    ...
    return dataGoodRef, dataGoodRef0


def scale_data(data, lat, lon, dataMeta, scale_method):
    dataGoodRef, dataGoodRef0 = validate_data(data, lat, lon)
    ...
    if scale_method == 'eds':
        ...
        radiance = data.copy()
        radiance[dataGoodRef0] = np.log10(radiance[dataGoodRef0])
        print("Log Min={:.2f} and Max={:.2f}".format(
            np.min(radiance[dataGoodRef0]), np.max(radiance[dataGoodRef0])))
    ...
    # %%
    # Scale data
    radiance = np.divide(np.subtract(radiance, minval), rangeval, dtype=np.double)
    radiance = np.where(radiance < 0., 0., radiance)
    radiance = radiance ** (1. / rootbase)  # np.sqrt(radiance)

    radMax = np.max(radiance)
    radMin = np.min(radiance)
    radiance = np.where(radiance > 1.0, 1.0, radiance)
    print("Scaled Radiance Minval= {:.5f} and Maxval={:.5f}".format(
        radMin, radMax))

    # %%
    # Created scaled data ranging from 0-255
    rad255 = radiance.copy() * 255
...
for i, case in enumerate(caseList):
    ...
    data, lat, lon, dataMeta = read_concat_files(caseDict)

    print(f'Case: ' + case + f', {i + 1} of {len(caseList)}')

    rad255 = scale_data(data, lat, lon, dataMeta, scale_method=scale_method)
    ...
    if i == 0:
        case_ds.to_zarr(zarr_savefn, mode='w')
    else:
        case_ds.to_zarr(zarr_savefn, append_dim='time', mode='a')

    del case_ds
    del rad255, lat, lon, dataMeta

    gc.collect()

with xr.open_zarr(zarr_savefn) as ds:
